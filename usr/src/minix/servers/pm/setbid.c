...
int do_setbid(void) /* so_2021 */
{
  ...
  // do not validate MAX_BID here, pass it to sched

  // mp = obecny proces (?)
  // -1 jeżeli zmienianie zawiodło

  if (bid < 0 || bid > MAX_BID) {
    mp->mp_reply.m_m1.m1i1 = -2;
  } else {
    mp->mp_reply.m_m1.m1i1 = 0;
  }

  return OK;
}

static int do_getsetpriority_hehe()
{
